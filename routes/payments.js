...
router.post('/', async (req, res, next) => {
  try {
    ...
    if (!distributorId || !amount || !date || !method) throw new AppError('distributorId, amount, date, method required', 400);
    ...
    const payment = await Payment.create({ distributorId, distributorName: dist.name, amount, date, method, reference: reference || '-', status: 'Pending' });
