...
export const createClient = async (req, res, next) => {
  try {
    const { name, type, phone, email, address, city, contactPerson, creditLimit } = req.body;

    if (!name || !type || !phone || !city || !creditLimit) {
      throw new AppError('Name, type, phone, city, and credit limit are required', 400);
    }
...
export const updateClient = async (req, res, next) => {
  try {
    const { id } = req.params;
    const { name, type, phone, email, address, city, contactPerson, creditLimit, status } = req.body;
